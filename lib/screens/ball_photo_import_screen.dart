...
class _BallPhotoImportScreenState extends State<BallPhotoImportScreen> {
  ...
  final _service = BallDetectionService();
  ...
  Future<void> _checkServer() async {
    final ok = await _service.isServerAvailable();
    if (!mounted) return;
    ...
    try {
      ...
      setState(() {
        ...
        if (layout.balls.isEmpty) {
          ...
          _status = '${layout.balls.length} 球を検出 ($mode · img=$imgSize)';
        }
      });
    ...
    const sample = '''
{
                ...
                children: [
                  ...
                  Row(
                    children: [
                      ...
                      const SizedBox(width: 8),
                      FilledButton(
                        onPressed: _result == null ? null : _applyAndReturn,
                        child: const Text('エディタへ反映'),
                      ),
                    ],
                  ),
                  if (_result != null)
  ...
  Widget _buildStatusBanner() {
    final apiText = _serverOk
        ? '検出 API: 接続 OK (127.0.0.1:8765)'
        : '検出 API: 未接続 — CLI JSON 貼り付け可';
    return Container(
      ...
      child: Column(
        ...
        children: [
          Text(apiText, style: const TextStyle(fontSize: 13)),
          if (_status != null)
            Padding(
  ...
  Widget _buildCornerPicker() {
    ...
    return Column(
      ...
      children: [
        Text(
          _corners.length < 4
              ? '次: ${_cornerLabels[_corners.length]}（スクロール可）'
              : '4点完了 — 緑の数値が API 送信座標',
          style: Theme.of(context).textTheme.titleSmall,
        ),
        const SizedBox(height: 6),
...
class _CornerOverlayPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (var i = 0; i < pts.length; i++) {
      canvas.drawCircle(pts[i], 8, Paint()..color = const Color(0xFF00E676));
      ...
      tp.paint(canvas, pts[i] + const Offset(10, -18));
    }
